...
/* -- servidor espera a recibir algo --- */
    ...
    while (1) {
        ...
        // recibe del socket
        rx_socket =
            recvfrom(SockEscucha, buffer_rx, sizeof(buffer_rx), 0,
                     (struct sockaddr *)&struct_direccion, &len);
        ...
        if (rx_socket > 0) {
            ...
            // envia al socket lo que recibio
            sendto(SockEscucha, buffer_rx, rx_socket, 0,
                   (struct sockaddr *)&struct_direccion,
                   sizeof(struct_direccion));
